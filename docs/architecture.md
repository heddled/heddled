...
**The workbench.** Three panes over one directory: `jarvis/work` is what the file tools write, what the shell is `cd`'d into, and where a script leaves its output — so the loop is write, run, read, fix. The file browser and the reader run in Heddled; the shell does not.
