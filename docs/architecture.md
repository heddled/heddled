...
The unit is a **conversation**, recorded in `jarvis_runs`. Each message is an ordinary turn on the ordinary spine — `submit_message` on the `jarvis` channel, streamed over SSE by the same client that drives `/chat` — so a conversation reads in Activity like any other session and needs no separate audit surface. There is no autonomous loop: a person answers every turn. `jarvis_max_steps` (default 8, ceiling 40) caps the tool iterations one message may take, passed to the engine as `max_iterations` with `soft_iteration_limit=True` — hitting it ends the turn through `_finish` with a plain "tell me to continue" rather than through `_fail`, because for Jarvis that is the dial the operator set and not a fault. Ordinary agents keep `config.MAX_TOOL_ITERATIONS` and the hard failure. The budget is the rail, checked before a turn rather than noticed after one, and it counts every session started underneath the conversation (a recursive CTE on `parent_session_id`) so `run_own_agent` cannot spend against a total that never moves.

Three fences carry it:

- **Its own tree.** `jarvis/agents`, `jarvis/tools`, `jarvis/memory`, `jarvis/work`, read through a second `Registry`. `runtime.registry_for(channel)` is the single place that decides, gated on the setting as well as the channel, and `_engine_for` passes the same registry into the turn so an agent and its tools always come from one tree. The operator's `agents/` and `tools/` are a different directory nothing in the module holds a path to — "it cannot edit your policies" is structural, not a rule it is asked to follow. Agent files are assembled field by field rather than written from what the model hands over, so `workspace` and `policies` have no path in at all, and neither does `triggers` — a schedule is added by its own tool (`make_schedule`), never as a field falling out of the call that made the agent.
...
**Scheduling.** Jarvis can put its own agents on a cron. `triggers.tick()` walks its registry as well as the operator's, and a Jarvis schedule enqueues with `channel: jarvis` so `fire_trigger` resolves the agent in the right tree and the engine still strips credentials — on the `schedule` channel it would look the agent up in the operator's estate and run with their keys. `kind` stays `schedule` in the origin, so the trace reads the same. Four rails, because nobody is watching: the driver is never schedulable (it holds the builders, and the tick skips it even if a file says otherwise); `_check_cron` refuses anything more often than every 15 minutes and anything that would never fire at all; `jarvis_schedule_budget_eur` caps what *everything* Jarvis schedules may spend in a day, and at zero none of it fires; and the whole set is skipped when Jarvis is switched off.

**The workbench.** Three panes over one directory: `jarvis/work` is what the file tools write, what the shell is `cd`'d into, and where a script leaves its output — so the loop is write, run, read, fix. It has a shape — `data/`, `scripts/`, `out/`, `tmp/`, created by `ensure_tree()` and described both in a README beside the files and in the driver's instructions — because one flat directory puts the script, its input and its output in a single list with nothing to distinguish them. Uploads land in `data/`. `workspace.listing()` skips dotfiles at any depth: a shell leaves `.python_history` and a `.cache` tree behind it, and the sandbox's `HOME` is container-local for the same reason — with it in the shared volume, `pip install` wrote its cache through the bind mount onto the operator's disk. Hiding is not fencing; `safe_path` is unchanged and a dotfile named directly still reads. Every file has a download, through `_as_download` — shared with the agent workspace route, so there is one place that decides a workspace file is served as `application/octet-stream`, `attachment` and `nosniff` rather than as a page on the origin holding the administrator's session. It builds the `Content-Disposition` filename rather than interpolating it: a name is whatever the filesystem accepts, and Jarvis writes the names, so a quote truncated the header and a newline made Werkzeug refuse it — a 500 on the download button for a file Jarvis had written. The file browser and the reader run in Heddled; the shell does not.

`sandbox/` is a second image and a second compose service, behind `--profile jarvis`. It mounts only `./jarvis/work`, publishes no port, declares no environment, and holds no Heddled — the fences survive a terminal because there is nothing in that container to reach. `heddled/jarvis_shell.py` speaks to it over the compose network and reports it as *not running* rather than failing when it is absent. Commands build their env as a literal rather than from `os.environ`, so a key set on the container is still invisible to what it runs; verified by setting one and looking. The server starts as root only long enough to take `/work` — bind-mounted from the host and therefore root-owned — then `setuid`s before serving, because an unprivileged container could not write to its own workspace.

`read_page` runs in Heddled, because what it needs is `tooltypes.guard_destination`: a model choosing a URL is the case that guard was written for. It is a reader, not a browser — no JavaScript — and what it returns is prefixed as somebody else's words, since a page saying "ignore your instructions" is a page.

`jarvis_console` records both, so a command the operator types sits in the same transcript as one the model ran. One shell, one directory, one history.
...
**It holds no credentials.** `TurnEngine.tool_settings` is what a tool may resolve `{{secret.name}}` against, and in a namespace that is not the operator's it is the settings with every credential removed. This is not a detail: a no-code tool is four lines of YAML, `{"type": "text", "config": {"text": "{{secret.anthropic_api_key}}"}}` returns the key as a tool result, and sandboxing the Python Jarvis writes protects nothing while the path we tell it to *prefer* is wide open. `make_tool` also refuses a manifest mentioning `{{secret.` — that changes no outcome, only the error message. `allow_internal_http` is withheld the same way: that setting is the operator saying *their* tools may reach their own network, and handing it on would give a model-chosen URL the run of 192.168.x.x and the cloud metadata endpoint. The engine still knows the real key, or it could not call a provider; only tools are scrubbed.
